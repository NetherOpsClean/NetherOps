...
describe("ResourceQuota Value Object", () => {
  describe("Creation and Validation", () => {
    ...
    it("should throw an error if the memory quota is zero", () => {
      expect(() => ResourceQuota.create(0)).toThrowError("Memory quota must be a positive integer");
    });

    it("should throw an error if the memory quota is a negative number", () => {
      expect(() => ResourceQuota.create(-512)).toThrowError(
        "Memory quota must be a positive integer"
      );
    });
  });
