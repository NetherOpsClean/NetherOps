...
const makeServer = (
  ...
  return Server.create(
    ...
    ServerConfiguration.create(20, "SURVIVAL", "NORMAL", true, "Test Server", false)
  );
};
...
describe("Server entity", () => {
  ...
  describe("reconstitute()", () => {
    it("should restore a server with the given status", () => {
      const server = Server.reconstitute(
        ...
        "RUNNING",
        25565,
        ServerConfiguration.create(20, "SURVIVAL", "NORMAL", true, "Test", false),
        new Date()
      );
  ...
  describe("changeConfiguration()", () => {
    it("should update configuration when server is OFFLINE", () => {
      const server = makeServer();
      const newConfig = ServerConfiguration.create(50, "CREATIVE", "EASY", false, "New MOTD", true);
      server.changeConfiguration(newConfig);
      expect(server.getConfiguration().maxPlayers).toBe(50);
      expect(server.getConfiguration().gameMode).toBe("CREATIVE");
    });
